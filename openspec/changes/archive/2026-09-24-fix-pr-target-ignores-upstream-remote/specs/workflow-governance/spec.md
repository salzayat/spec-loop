...
commit regardless of whether the caller staged all changes or named explicit paths. When the current user has no write access to the repository, the automation MUST push the branch to a fork of it owned by that user, creating the fork when it does not exist, and MUST open the pull request against the upstream repository with the fork's branch as its head; it MUST NOT attempt a push to the repository itself that the user's permissions cannot satisfy. A user with write access MUST continue to push to the repository directly. The repository a pull request targets MUST be resolved from the `origin` remote (its parent when `origin` is a fork, itself otherwise), passed to the GitHub CLI explicitly rather than left to the CLI's own remote preference, so that an `upstream` remote added for template tracking never redirects the push or the pull request.
...
#### Scenario: An upstream remote does not redirect the pull request

- GIVEN a checkout whose `origin` is the project's own repository and which also has an `upstream` remote
  pointing at the template it was forked from
- WHEN the contributor runs the PR helper
- THEN the pull request targets the repository `origin` points at
- AND no branch is pushed to `upstream` and no fork of the template is created
